...
class ArcConverting {
    ...
    svgArcToCenterParam(x1, y1, rx, ry, degree, fA, fS, x2, y2) {
    ...
    closestPointOnCircle(cx, cy, r, x, y) {
    ...
    convertEllipticalToCircularArc(x1, y1, rx, ry, flag1, flag2, flag3, x2, y2) {
    ...
    approximateEllipse(a, b, cx, cy, segments, clockwise = true, startAngle = 0, endAngle = 2 * Math.PI, deltaAngle) {
        let path = '';
        const direction = clockwise ? 1 : 1;
        const totalTheta = deltaAngle        
        segments = Math.ceil (Math.abs(  (deltaAngle * segments) / (Math.PI*2)  ))
    ...
    findRadius(arcLength, chordLength, x1, y1, rx, ry, flag1, flag2, flag3, x2, y2) {
        function f(R) {
    ...
    arcConvert(a, b, cx, cy, clockwise = false, startAngle=0, endAngle=2*Math.PI, deltaAngle=2*Math.PI) {
        let segments = 24//48
